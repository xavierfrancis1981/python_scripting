# Visualizing weather data from OpenWeatherMap
...
# Main
def main():
    load_dotenv(dotenv_path="openweather_api_key.env")  # Load from .env file
    API_KEY = os.getenv("OPENWEATHER_API_KEY")
    # city = input("Enter city name (default London): ").strip() or "London"
    cities = ["London", "Birmingham", "Edinburgh", "Paris", "Milan", "Oslo"]
    for city in cities:

        try:
            forecast = get_forecast(city, API_KEY)
            if forecast.get("cod") != "200":
                print("Error:", forecast.get("message", "Failed to fetch forecast."))
                return
            ...
            for day in temp_avg:
                temp = temp_avg[day]
                wind = wind_avg.get(day, 0.0)  # default to 0.0 if missing
...
```
### Example plots
![alt-text](./London_forecast.png "optional-title")
![alt-text](./Birmingham_forecast.png "optional-title")
![alt-text](./Edinburgh_forecast.png "optional-title")
![alt-text](./Paris_forecast.png "optional-title")
![alt-text](./Milan_forecast.png "optional-title")
![alt-text](./Oslo_forecast.png "optional-title")
